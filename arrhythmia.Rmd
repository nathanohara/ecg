...
```{r arr-ecg-simulated, fig.width = 15, fig.height = 8}
for (i in 1:5){
  x <- ggplot() +
    geom_line(aes(x = ARR_TIME[[i]], y = unlist(ARR_DF[[i]][,2]), col = "blue"), size = 2) +
    geom_line(aes(x = ARR_TIME[[i]], y = unlist(ARR_DF[[i]][,3]), col = "orange"), size = 2) +
...
for(j in sample(12, 3)){
  plot(unlist(ARR_ALPHA[[2]][j]), type = 'l', ylab = paste("Alpha", j), xlab = "Iter", col = "purple", main = paste("Acceptance ratio =", round(length(unique(unlist(ARR_ALPHA[[2]][j]))) / S, 2), "   ", "ESS = ", round(effectiveSize(unlist(ARR_ALPHA[[1]][j])),2)))
  acf(unlist(ARR_ALPHA[[2]][3]), ylab = paste("Alpha", j), main = "")
}

par(mfrow = c(3, 2))
for(j in sample(17, 3)){
  plot(unlist(ARR_DELTA[[2]][j]), type = 'l', ylab = paste("Delta", j), xlab = "Iter", col = "purple", main = paste("Acceptance ratio =", round(length(unique(unlist(ARR_DELTA[[2]][j]))) / S, 2), "   ", "ESS = ", round(effectiveSize(unlist(HEALTHY_DELTA[[2]][j])),2)))
...
plot(unlist(ARR_DC[[2]]), type = 'l', ylab = "DC", xlab = "Iter", col = "purple",
       main = paste("Acceptance ratio =", round(length(unique(unlist(ARR_DC[[2]]))) / S, 2), " ", "ESS = ", round(effectiveSize(unlist(HEALTHY_DELTA[[2]][j])),2)))
acf(unlist(ARR_DC[[2]]), ylab = "DC", main = "")

par(mfrow = c(1,2))
plot(unlist(ARR_TAO[[2]]), type = 'l', ylab = "Tao", xlab = "Iter", col = "purple",
    main = paste("Acceptance ratio =", round(length(unique(unlist(ARR_TAO[[2]]))) / S, 2), "  ", "ESS = ", round(effectiveSize(unlist(HEALTHY_DELTA[[2]][j])),2)))
